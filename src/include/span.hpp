...
#define BUG(span, msg)  do { std::ostringstream ss; ss << msg; ::Span(span).bug([&](::std::ostream& os) {os << __FILE__ << ":" << __LINE__ << ": " << ss.str(); }); throw ::std::runtime_error("Bug fell through");} while(0)
